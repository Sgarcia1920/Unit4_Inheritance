namespace Unit4_Inheritance
{
	partial class Form1
	{
		/// <summary>
		///  Required designer variable.
		/// </summary>
		private System.ComponentModel.IContainer components = null;

		/// <summary>
		///  Clean up any resources being used.
		/// </summary>
		/// <param name="disposing">true if managed resources should be disposed; otherwise, false.</param>
		protected override void Dispose(bool disposing)
		{
			if (disposing && (components != null))
			{
				components.Dispose();
			}
			base.Dispose(disposing);
		}

		#region Windows Form Designer generated code

		/// <summary>
		///  Required method for Designer support - do not modify
		///  the contents of this method with the code editor.
		/// </summary>
		private void InitializeComponent()
		{
			label1 = new Label();
			label2 = new Label();
			btn_teacher = new Button();
			button1 = new Button();
			SuspendLayout();
			// 
			// label1
			// 
			label1.AutoSize = true;
			label1.Font = new Font("Times New Roman", 21.75F, FontStyle.Regular, GraphicsUnit.Point, 0);
			label1.ForeColor = Color.Navy;
			label1.Location = new Point(114, 116);
			label1.Name = "label1";
			label1.Size = new Size(304, 33);
			label1.TabIndex = 0;
			label1.Text = "Welcome to Choose User";
			label1.TextAlign = ContentAlignment.MiddleCenter;
			// 
			// label2
			// 
			label2.AutoSize = true;
			label2.Font = new Font("Segoe UI Emoji", 24F, FontStyle.Regular, GraphicsUnit.Point, 0);
			label2.ForeColor = Color.DarkCyan;
			label2.Location = new Point(168, 61);
			label2.Name = "label2";
			label2.Size = new Size(190, 43);
			label2.TabIndex = 1;
			label2.Text = "Registration";
			// 
			// btn_teacher
			// 
			btn_teacher.BackColor = Color.Navy;
			btn_teacher.Font = new Font("Segoe UI Historic", 14.25F, FontStyle.Regular, GraphicsUnit.Point, 0);
			btn_teacher.ForeColor = SystemColors.ButtonFace;
			btn_teacher.Location = new Point(126, 168);
			btn_teacher.Name = "btn_teacher";
			btn_teacher.Size = new Size(120, 56);
			btn_teacher.TabIndex = 2;
			btn_teacher.Text = "Teacher";
			btn_teacher.UseVisualStyleBackColor = false;
			btn_teacher.Click += btn_teacher_Click;
			// 
			// button1
			// 
			button1.BackColor = Color.Navy;
			button1.Font = new Font("Segoe UI Historic", 14.25F, FontStyle.Regular, GraphicsUnit.Point, 0);
			button1.ForeColor = SystemColors.ButtonFace;
			button1.Location = new Point(287, 168);
			button1.Name = "button1";
			button1.Size = new Size(120, 56);
			button1.TabIndex = 3;
			button1.Text = "Student";
			button1.UseVisualStyleBackColor = false;
			button1.Click += button1_Click;
			// 
			// Form1
			// 
			AutoScaleDimensions = new SizeF(7F, 15F);
			AutoScaleMode = AutoScaleMode.Font;
			BackColor = Color.AliceBlue;
			ClientSize = new Size(520, 313);
			Controls.Add(button1);
			Controls.Add(btn_teacher);
			Controls.Add(label2);
			Controls.Add(label1);
			ForeColor = SystemColors.ActiveCaption;
			Name = "Form1";
			StartPosition = FormStartPosition.CenterScreen;
			Text = "Login";
			ResumeLayout(false);
			PerformLayout();
		}

		#endregion

		private Label label1;
		private Label label2;
		private Button btn_teacher;
		private Button button1;
	}
}
